class_name GPUDataVector2Base
extends GPUDataBase
tool

# Getters
func get_default_resource_name() -> String:
	return 'GPUDataVector2Base'
...
func read_data_internal() -> Color:
	var data_array = get_data_array()
	return Color(data_array[data_ptr].x, data_array[data_ptr].y, 0.0, 1.0)

func get_data(index: int) -> Color:
	var data = get_data_array()[index]
	return Color(data.x, data.y, 0.0, 1.0)
